...
export const ReplaceDataSeries = ({ match: { params } }: Props) => {
  ...
  useEffect(
    () => {
      if (params && params.id !== -1) {
        client.getDataSeries(params.id)
          .then((result: DataSeries) => {
            ...
            if (result.dataConnection) {
              setDataConnection(result.dataConnection);
            }
          })
  ...
  const onSubmitDataSeriesSuccess = () => {
    client.deleteDataSeries(params.id);
  };
