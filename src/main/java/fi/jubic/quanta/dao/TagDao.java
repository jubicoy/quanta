...
public class TagDao {
    ...
    public List<TagAssignment> getAllTaskTags(List<Long> taskIds) {
        return DSL.using(conf)
                ...
                .fetchStream()
                .collect(TagAssignment.tagTaskMapper.withTag(Tag.mapper));
    }
    ...
    public List<TagAssignment> getAllDataConnectionTags(List<Long> dataConnIds) {
        return DSL.using(conf)
                ...
                .fetchStream()
                .collect(TagAssignment.tagDataConnectionMapper.withTag(Tag.mapper));
    }
    ...
    public Task enrichTaskTags(Task task) {
        Set<String> tags = getTaskTags(task.getId())
                .stream()
                .map(Tag::getName)
    ...
    public DataConnection enrichDataConnectionTags(DataConnection dataConnection) {
        Set<String> tags = getDataConnectionTags(dataConnection.getId())
                .stream()
                .map(Tag::getName)
                .collect(Collectors.toSet());
    ...
    public List<Tag> updateTaskTags(Long taskId, Set<String> tagNames) {
        ...
        Set<String> tagsToRemove = oldTags
                .stream()
                .filter(Predicate.not(tagNames::contains))
                .collect(Collectors.toSet());
    ...
    public List<Tag> updateDataConnectionTags(Long dataConnectionId, Set<String> tagNames) {
        Set<String> oldTags = DSL.using(conf)
                .select()
                .from(TAG)
                .leftJoin(TAG_DATACONNECTION)
